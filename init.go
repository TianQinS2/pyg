...
// SetProgramName should be called before Py_Initialize() is called for
...
// GetProgramName returns the program name set with Py_SetProgramName(),
// or the default.
...
// Sys_SetArgv initializes the 'sys.argv' array in python.
func Sys_SetArgv(argv []string) {
	argc := C.int(len(argv))
...
// SetPythonHome Sets the default home directory, that is,
...
// GetPythonHome returns the default home, that is, the value set by
// a previous call to SetPythonHome(), or the value
